...
class VMManager(object):
    ...
    def __init__(self, name=None):
        """
        *name* is the name of the VM to manage. If omitted, the current VM set in the config will
        be used.
        """
        self._config = vbox.config.Config()
        self.name = name if name is not None else self._config.get_current_vm()['name']

    def start_and_connect(self, address=None, tries=10, retry_interval=5):
        """
        Powers on the VM if it is not already and then connects via SSH

        *address* is the IP address or hostname of the VM to which to connect. If omitted, the
        address associated with the VM's name in the config will be used.

        *tries* is the number of times to try connecting to the VM before timing out (default: 10)

        *retry_interval* is the number of seconds to wait before each retry (default: 5)
        """
        self.start()
        self.connect(address=address, tries=tries, retry_interval=retry_interval)
    ...
    def start(self):
        ...
        if self.is_running():
            ...
            subprocess.check_call(['VBoxManage', 'startvm', self.name, '--type', 'headless'])
    ...
    def get_state(self):
        ...
        vm_state_match = re.search(r'^VMState="(.*)"$', vm_info, flags=re.MULTILINE)

        if vm_state_match is None:
            raise VBoxManageOutputParseError('Could not state of VM {}'.format(self.name))

        return vm_state_match.group(1)

    def connect(self, address=None, tries=10, retry_interval=5):
        """
        ...
        *retry_interval* is the number of seconds to wait before each retry (default: 5)
        """
        address = address if address is not None else self._config.get_vm(self.name)['address']
    ...
    def stop(self):
        ...
        if self.is_running():
            ...
            subprocess.check_call(['VBoxManage', 'controlvm', self.name, 'acpipowerbutton'])
        else:
